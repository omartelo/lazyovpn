...
- Auto-reconnect re-invokes `pkexec`, so polkit may re-prompt for the root password on a redial once its auth cache (`auth_admin_keep`, ~5 min) expires — it is not a silent background reconnect after that window. It is also off for `daemon` configs (untrackable), capped at `maxReconnects`, and on by default with no in-app toggle (press `d` to cancel a pending one).
